...
from .serializers import (
    ...
    OptimizedFuelPlanRequestSerializer,
)
from .services.routing_service import (
    get_coordinates,
    get_distance_and_duration_and_route_geometry,
)
...
def get_route_data(request):
    ...
    try:
        ...
        if "error" in start_coords:
            return Response(
                {"error": f"Failed to get coordinates for start location: {start}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        end_coords = get_coordinates(end)
        if "error" in end_coords:
            return Response(
                {"error": f"Failed to get coordinates for end location: {end}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {"error": "An error occurred while processing the route data"},
...
def candidate_stations(request):
    ...
    try:
        ...
        stations = get_station_near_route(
        ...
        return Response(
            {"error": "An error occurred while processing candidate stations"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )


@api_view(["POST"])
@timeit("optimize_fuel_plan")
def optimize_fuel_plan(request):
    ...
    try:
        ...
        if route_distance_miles is None:
            return Response(
                {"error": "Route distance information is missing"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        candidate_stations = get_station_near_route(
            route_geometry=route_geometry, threshold_miles=10, sample_rate=100
        )
        ...
        response_data = {
            ...
            "fuel_summary": {
                ...
                "total_gallons_purchased": round(
                    optimized_plan["total_gallons_purchased"], 2
                ),
        ...
        return Response(
            {"error": "An error occurred while optimizing the fuel plan"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
